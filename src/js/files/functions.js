...
export function menuInit() {
  ...
  let menuLinks = document.querySelectorAll('.menu__list .menu-item a');
  const html = document.documentElement;
  const menuWrapper = document.querySelector('.menu__wrapper');
  const DESKTOP_W = 1470;
  ...
  // Закрытие по клику вне меню на ≥1470
  document.addEventListener('click', (e) => {
    if (!html.classList.contains('menu-open')) return;
    if (window.innerWidth < DESKTOP_W) return;
    const clickedInside =
      menuWrapper?.contains(e.target) || iconMenu?.contains(e.target);
    if (!clickedInside) {
      bodyUnlock();
